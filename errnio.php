...
Description: errnio Mobile WordPress Site Search plugin is a mobile-first search box for your site, with far better engagement for internal site search.
Version: 2.6
Author: Errnio
...
define('SEARCHY_SITE_BY_ERRNIO_VERSION', '2.6');
...
define('SEARCHY_SITE_BY_ERRNIO_OPTION_NAME_NOTIFICATION', 'errnio_notification_status');
...
function searchy_site_by_errnio_deactivate() {
	...
	delete_option(SEARCHY_SITE_BY_ERRNIO_OPTION_NAME_NOTIFICATION);
}
...
function searchy_site_by_errnio_uninstall() {
	...
	$plugins = searchy_site_by_errnio_multiple_plugins();
...
function searchy_site_by_errnio_get_first_post_image( $postID ) {
    $args = array(
    'numberposts' => 1,
    'order' => 'ASC',
...
function searchy_site_by_errnio_load_client_script() {
    ...
    wp_register_script($handle, $script_url, false, SEARCHY_SITE_BY_ERRNIO_VERSION, true);
    ...
    if ( !empty($next_post) ) {
        ...
        $next_post_thumb = searchy_site_by_errnio_get_first_post_image($next_post->ID);
        if ($next_post_thumb) {
    ...
    if ( !empty($prev_post) ) {
        ...
        $prev_post_thumb = searchy_site_by_errnio_get_first_post_image($prev_post->ID);
        if ($prev_post_thumb) {
...
function searchy_site_by_errnio_add_settings_menu_option() {
    add_menu_page (
        ...
        'Site Search Settings',  //menu title
        'manage_options',   //capability
        'errnio-options-searchy-site',   //menu_slug
...
function searchy_site_by_errnio_add_settings_link_on_plugin($links, $file) {
    ...
    if ($file == $this_plugin) {
		$adminpage_url = admin_url( 'admin.php?page=errnio-options-searchy-site' );
...
function searchy_site_by_errnio_admin_notice() {
	...
	$settingsurl = admin_url( 'admin.php?page=errnio-options-searchy-site' );

	$shouldshow = get_option(SEARCHY_SITE_BY_ERRNIO_OPTION_NAME_NOTIFICATION);

	if($hook_suffix == 'plugins.php' && $needregister && $shouldshow != 'hide'){
		$plugin_name = "Mobile Site Search";
		$message_id = "mobile-site-search-errnio";
...
function searchy_site_by_errnio_close_notification() {
	add_option(SEARCHY_SITE_BY_ERRNIO_OPTION_NAME_NOTIFICATION, 'hide');
}

function searchy_site_by_errnio_multiple_plugins() {
...
function searchy_site_by_errnio_admin_page() {
    ...
    <div class="errnio-settings-wrap">
		...
		if (!$needregister) {
			...
			if ($tagId) {
				echo '<div id="errnioSettingsAdmin" class="errnio-settings-screen" data-tagId="'.$tagId.'" data-installName="'.SEARCHY_SITE_BY_ERRNIO_INSTALLER_NAME.'">';
...
add_action('wp_ajax_searchy_site_by_errnio_close_notification', 'searchy_site_by_errnio_close_notification');